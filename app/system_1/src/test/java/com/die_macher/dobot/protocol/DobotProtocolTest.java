...
/**
 * Unit-Tests für die DobotProtocol-Klasse.
 * Diese Tests überprüfen die Funktionalität der Protokollkonstanten und -methoden.
 */
public class DobotProtocolTest {
    ...
    public void testValidateChecksum() {
        ...
        // Act & Assert
        assertTrue(DobotProtocol.validateChecksum(validResponse), "Gültige Prüfsumme sollte validiert werden");
        assertFalse(DobotProtocol.validateChecksum(invalidResponse), "Ungültige Prüfsumme sollte nicht validiert werden");
    }
    ...
    public void testValidateResponseFormat() {
        ...
        byte[] invalidHeader = new byte[] {
            (byte)0xBB, (byte)0xBB, // Falscher Header
            0x05,                   // Länge
            0x00,                   // Befehl
            0x00,                   // Steuerung
            0x10, 0x20, 0x30,       // Payload
        ...
        // Act & Assert
        assertTrue(DobotProtocol.validateResponseFormat(validResponse, expectedCommand), 
                "Gültiges Nachrichtenformat sollte validiert werden");
        assertFalse(DobotProtocol.validateResponseFormat(invalidHeader, expectedCommand), 
                "Nachricht mit ungültigem Header sollte nicht validiert werden");
        assertFalse(DobotProtocol.validateResponseFormat(invalidCommand, expectedCommand), 
                "Nachricht mit falschem Befehl sollte nicht validiert werden");
        assertFalse(DobotProtocol.validateResponseFormat(tooShort, expectedCommand), 
                "Zu kurze Nachricht sollte nicht validiert werden");
        assertFalse(DobotProtocol.validateResponseFormat(null, expectedCommand), 
    ...
    public void testExtractResponsePayloadWithInvalidResponse() {
        ...
        // Act & Assert
        assertNull(DobotProtocol.extractResponsePayload(null), 
                "Null-Antwort sollte null zurückgeben");
        assertNull(DobotProtocol.extractResponsePayload(tooShort), 
                "Zu kurze Antwort sollte null zurückgeben");
        assertNull(DobotProtocol.extractResponsePayload(invalidLength), 
    ...
    public void testCreateControlByte(boolean isWrite, boolean isQueued, byte expected) {
        ...
        // Assert
        assertEquals(expected, controlByte, 
                String.format("Steuerungsbyte für (isWrite=%s, isQueued=%s) sollte 0x%02X sein", 
