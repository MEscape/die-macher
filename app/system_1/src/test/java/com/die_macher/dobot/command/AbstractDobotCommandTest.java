...
/**
 * Unit tests for the AbstractDobotCommand class.
 * Tests focus on exception scenarios in the execute method.
 */
@ExtendWith(MockitoExtension.class)
public class AbstractDobotCommandTest {
    ...
    public void testNotConnectedToDobot() {
        // Arrange
        when(connector.isConnected()).thenReturn(false);

        // Act & Assert
        DobotCommunicationException exception = assertThrows(DobotCommunicationException.class,
                () -> command.execute(connector),
                "Should throw exception when not connected");

        assertEquals("Not connected to Dobot", exception.getMessage(),
    ...
    public void testFailedToSendCommand() {
        // Arrange
        ...
        assertEquals("Failed to send command to Dobot", exception.getMessage(),
    ...
    public void testNoResponseReceived() {
        // Arrange
        when(connector.isConnected()).thenReturn(true);
        ...
        assertEquals("No response received for command", exception.getMessage(),
    ...
    public void testEmptyResponseReceived() {
        // Arrange
        when(connector.isConnected()).thenReturn(true);
        when(connector.sendData(any(byte[].class))).thenReturn(true);
     ...
     */
    private class TestDobotCommand extends AbstractDobotCommand<String> {
