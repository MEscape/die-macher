...
class AwattarServiceImplTest {
  ...
  @Mock private AwattarApiClient apiClient;

  @Mock private AwattarMapper mapper;

  @Mock private AwattarConfig config;

  @InjectMocks private AwattarServiceImpl awattarService;
  ...
  void testInitOnStartup_AfterOnePM_DataAvailable() {
    // Arrange
    AwattarServiceImpl testService =
        new AwattarServiceImpl(apiClient, mapper, config) {
          @Override
  ...
  void testCalculateOptimalProductionWindow_Success() {
    ...
    OptimalProductionWindow window =
        (OptimalProductionWindow) ReflectionTestUtils.getField(awattarService, "optimalWindow");
  ...
  void testCalculateProductionCost_WithEmptyPriceList() {
    ...
    // Act - Methode über Reflection aufrufen
    double cost =
        ReflectionTestUtils.invokeMethod(awattarService, "calculateProductionCost", emptyPrices);
  ...
  void testCalculateProductionCost_WithNormalPriceList() {
    ...
    double cost =
        ReflectionTestUtils.invokeMethod(awattarService, "calculateProductionCost", prices);
