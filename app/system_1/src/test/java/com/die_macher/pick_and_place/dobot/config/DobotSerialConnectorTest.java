...
class DobotSerialConnectorTest {
  ...
  void testConnectSuccess() {
    ...
    try (MockedStatic<SerialPort> serialPortMockedStatic = mockStatic(SerialPort.class)) {
      ...
      serialPortMockedStatic
          .when(SerialPort::getCommPorts)
          .thenReturn(new SerialPort[] {serialPort});
      ...
      verify(serialPort)
          .setComPortTimeouts(SerialPort.TIMEOUT_READ_SEMI_BLOCKING, timeout, timeout);
  ...
  void testReadResponseSuccess() {
    ...
    byte[] responseData = {
      ...
      0x00,
      0x01,
      (byte) ((byte) 0xAA + (byte) 0xBB + 0x03 + 0x01 + 0x01)
    };
