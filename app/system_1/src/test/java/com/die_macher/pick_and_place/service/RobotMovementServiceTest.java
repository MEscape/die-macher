...
class RobotMovementServiceTest {
    ...
    void shouldPickupCube() {
        ...
        when(heightCalculator.calculateLiftHeight(stackPosition)).thenReturn(30.0f);
        ...
        verify(dobotService, times(3)).moveToPosition(anyFloat(), anyFloat(), anyFloat(), anyFloat());
    ...
    void shouldMoveToCamera() {
        ...
        verify(dobotService).moveToPosition(cameraPoint.x(), cameraPoint.y(), cameraPoint.z(), cameraPoint.r());
        verify(dobotService).executeQueue();
    }
    ...
    void shouldPlaceCubeForRedColor() {
        ...
        when(heightCalculator.calculateLiftHeight(stackHeight)).thenReturn(20.0f);

        // When
        robotMovementService.placeCube(color, stackHeight);

        // Then
        verify(dobotService).stopExecuteQueue();
        verify(dobotService).clearQueue();
        verify(dobotService).setVacuumState(false);
        verify(dobotService, times(4)).moveToPosition(anyFloat(), anyFloat(), anyFloat(), anyFloat());
        verify(dobotService).executeQueue();
    }
    ...
    void shouldHandleAllSupportedColors() {
        ...
        when(heightCalculator.calculateLiftHeight(anyInt())).thenReturn(20.0f);

        // When & Then
        robotMovementService.placeCube(Color.RED, 0);
        robotMovementService.placeCube(Color.GREEN, 0);
        robotMovementService.placeCube(Color.BLUE, 0);
        robotMovementService.placeCube(Color.YELLOW, 0);

        verify(dobotService, times(4)).executeQueue();
    }

    @Test
    @DisplayName("Should throw exception for unsupported color")
    void shouldThrowExceptionForUnsupportedColor() {
        // Given
        Color unsupportedColor = Color.ORANGE;

        // Then
        assertThrows(IllegalArgumentException.class,
                () -> robotMovementService.placeCube(unsupportedColor, 0));
    }
}