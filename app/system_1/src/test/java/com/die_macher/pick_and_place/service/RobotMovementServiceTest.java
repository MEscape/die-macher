...
class RobotMovementServiceTest {
  ...
  @Mock private DobotService dobotService;

  @Mock private RobotConfiguration config;

  @Mock private HeightCalculator heightCalculator;

  @Mock private RobotConfiguration.RobotPositions positions;

  @Mock private RobotConfiguration.MovementProfile fastMovement;

  @Mock private RobotConfiguration.MovementProfile slowMovement;
  ...
  void shouldInitializeRobot() {
    ...
    verify(dobotService)
        .setDefaultHome(startPoint.x(), startPoint.y(), startPoint.z(), startPoint.r());
    verify(dobotService)
        .setMovementConfig(
  ...
  void shouldPickupCube() {
    ...
    // Then
    verify(dobotService, times(2))
        .setMovementConfig(anyFloat(), anyFloat(), anyFloat(), anyFloat());
    verify(dobotService, times(1))
        .moveToPosition(eq(PTPModes.MOVL_XYZ), anyFloat(), anyFloat(), anyFloat(), anyFloat());
    verify(dobotService, times(1))
        .moveToPosition(eq(PTPModes.MOVJ_XYZ), anyFloat(), anyFloat(), anyFloat(), anyFloat());
  ...
  void shouldMoveToCamera() {
    ...
    verify(dobotService)
        .moveToPosition(
            PTPModes.JUMP_XYZ, cameraPoint.x(), cameraPoint.y(), cameraPoint.z(), cameraPoint.r());
    verify(dobotService).executeQueue();
  }
  ...
  void shouldPlaceCubeForRedColor() {
    ...
    verify(dobotService, times(1))
        .moveToPosition(eq(PTPModes.MOVL_XYZ), anyFloat(), anyFloat(), anyFloat(), anyFloat());
    verify(dobotService, times(2))
        .moveToPosition(eq(PTPModes.JUMP_XYZ), anyFloat(), anyFloat(), anyFloat(), anyFloat());
    verify(dobotService).executeQueue();
  }
