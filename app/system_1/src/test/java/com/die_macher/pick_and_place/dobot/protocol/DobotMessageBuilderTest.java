...
class DobotMessageBuilderTest {
  ...
  void testBuildMessageWithoutPayload() {
    ...
    // Act
    byte[] message = DobotMessageBuilder.command(command).control(isWrite, isQueued).build();
    ...
    assertEquals(
        ...
        "Die Länge sollte 2 sein (Befehl + Steuerung)");
    assertEquals(
        command.getValue(),
        message[DobotProtocol.Indices.COMMAND_INDEX],
        "Der Befehlsindex sollte korrekt sein");
    assertEquals(
        0x00, message[DobotProtocol.Indices.CONTROL_INDEX], "Der Steuerungsbyte sollte 0x00 sein");

    // Überprüfe die Prüfsumme
    byte expectedChecksum =
        DobotProtocol.calculateChecksum(
            new byte[] {message[0], message[1], message[2], message[3], message[4]});
    assertEquals(
        expectedChecksum, message[message.length - 1], "Die Prüfsumme sollte korrekt sein");
  }
  ...
  void testBuildMessageWithPayload() {
    ...
    // Act
    byte[] message =
        DobotMessageBuilder.command(command).control(isWrite, isQueued).payload(payload).build();
    ...
    assertEquals(
        ...
        "Der Befehlsindex sollte korrekt sein");
    assertEquals(
        0x03, message[DobotProtocol.Indices.CONTROL_INDEX], "Der Steuerungsbyte sollte 0x03 sein");

    // Überprüfe die Payload
    for (int i = 0; i < payload.length; i++) {
      assertEquals(
          payload[i],
          message[DobotProtocol.Indices.PAYLOAD_INDEX + i],
  ...
  void testBuildMessageWithNullPayload() {
    ...
    // Act
    byte[] message =
        DobotMessageBuilder.command(command).control(isWrite, isQueued).payload(null).build();
    ...
    assertEquals(
        ...
        "Der Befehlsindex sollte korrekt sein");
    assertEquals(
        0x01, message[DobotProtocol.Indices.CONTROL_INDEX], "Der Steuerungsbyte sollte 0x01 sein");
    assertEquals(
        6,
        message.length,
        "Die Gesamtlänge sollte 6 sein (Header + Länge + Befehl + Steuerung + Prüfsumme)");
  }
  ...
  void testControlByteVariations() {
    // Arrange & Act
    byte[] message1 =
        DobotMessageBuilder.command(DobotProtocol.Commands.GET_DEVICE_SN)
            .control(false, false)
            .build();
    ...
    assertEquals(
        0x00,
        message1[DobotProtocol.Indices.CONTROL_INDEX],
        "Steuerungsbyte für (false, false) sollte 0x00 sein");
    assertEquals(
        0x01,
        message2[DobotProtocol.Indices.CONTROL_INDEX],
        "Steuerungsbyte für (true, false) sollte 0x01 sein");
    assertEquals(
        0x02,
        message3[DobotProtocol.Indices.CONTROL_INDEX],
        "Steuerungsbyte für (false, true) sollte 0x02 sein");
    assertEquals(
        0x03,
        message4[DobotProtocol.Indices.CONTROL_INDEX],
        "Steuerungsbyte für (true, true) sollte 0x03 sein");
  }
  ...
  void testFluentApiChaining() {
    ...
    byte[] message2 =
        DobotMessageBuilder.command(command).payload(payload).control(isWrite, isQueued).build();

    // Beide Nachrichten sollten identisch sein
    assertArrayEquals(
        message1,
        message2,
        "Die Reihenfolge der Methodenaufrufe sollte das Ergebnis nicht beeinflussen");
  }
}
