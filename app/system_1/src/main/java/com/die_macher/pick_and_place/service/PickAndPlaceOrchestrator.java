...
public class PickAndPlaceOrchestrator {
    ...
    private static final int ROBOT_STABILIZATION_TIME_MS = 35000;
    ...
    private final ConcurrentHashMap<Integer, CompletableFuture<Color>> pendingDetections = new ConcurrentHashMap<>();

    @Autowired
    public PickAndPlaceOrchestrator(RobotMovementService robotMovementService,
    ...
    private void processCube(int cubePosition) {
        try {
            ...
            Thread.sleep(ROBOT_STABILIZATION_TIME_MS);

            // Request color detection
            Color detectedColor = requestColorDetection(cubePosition);

            // Update stack and place cube
            StackInfo stackInfo = stackTracker.addCube(detectedColor);
            robotMovementService.placeCube(detectedColor, stackInfo.currentHeight());
    ...
    private Color requestColorDetection(int cubeId) {
        CompletableFuture<Color> colorFuture = new CompletableFuture<>();
        pendingDetections.put(cubeId, colorFuture);

        // Publish event to request color detection
        eventPublisher.publishEvent(new ImageRequestedEvent(this, cubeId));

        try {
            return colorFuture.get(COLOR_DETECTION_TIMEOUT_SECONDS, TimeUnit.SECONDS);
        } catch (Exception e) {
            pendingDetections.remove(cubeId);
            throw new RuntimeException("Color detection timeout or failed for cube " + cubeId, e);
        }
    }
    ...
    public void handleColorDetected(ImageReceivedEvent event) {
        ...
        LOGGER.info("Color detected: {} for cube {}", detectedColor, event.getCubeId());

        CompletableFuture<Color> future = pendingDetections.remove(event.getCubeId());
