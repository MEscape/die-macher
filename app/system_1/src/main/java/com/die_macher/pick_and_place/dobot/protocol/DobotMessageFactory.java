...
/**
 * Factory class for creating Dobot protocol messages. This class provides various methods to
 * generate properly formatted message packets for different commands to be sent to the Dobot
 * device. Each method corresponds to a specific Dobot command as defined in the protocol.
 */
public class DobotMessageFactory {
  private DobotMessageFactory() {}

  /**
   * Creates a message to retrieve the Device Serial Number (SN) from the Dobot. This is a read-only
   * command and does not require a payload.
   *
  ...
  /**
   * Creates a message to retrieve the Device Name from the Dobot. This is a read-only command and
  ...
  /**
   * Creates a message to move the Dobot to a specified position (x, y, z, r). The coordinates are
   * serialized in little-endian format as floats.
   *
   ...
   */
  public static byte[] createSetPTPCmdMessage(
  ...
  /**
   * Creates a message to set the Dobot to its home position. This is a queued command, and the
   * payload is reserved (4 bytes set to zero).
   *
   * @param isQueued Whether the command should be queued for execution.
  ...
  /**
   * Creates a message to toggle the end effector suction cup on or off. The state of suction is
   * represented as a boolean and serialized as bytes.
   *
  ...
  /**
   * Creates a message to set the home parameters of the Dobot. This sets the coordinates the Dobot
   * considers its home position.
   *
   * @return A byte array containing the complete message for SET_HOME_PARAMS.
   */
  public static byte[] createSetHomeParamsMessage(
      float x, float y, float z, float r, boolean isQueued) {
    // Allocate a ByteBuffer with 16 bytes (4 floats * 4 bytes each)
   ...
   */
  public static byte[] createSetPTPCoordinateParamsMessage(
      float xyzVelocity,
      float rVelocity,
   ...
   */
  public static byte[] createSetPTPJumpParamsMessage(
      float jumpHeight, float maxHeight, boolean isQueued) {
