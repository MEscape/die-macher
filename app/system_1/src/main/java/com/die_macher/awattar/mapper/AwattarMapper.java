...
public class AwattarMapper {
  ...
  public MarketDataDto toDto(MarketData marketData) {
    ...
    if (marketData.getData() != null) {
      List<MarketPriceDto> priceDtos =
          marketData.getData().stream().map(this::toDto).collect(Collectors.toList());
  ...
  public MarketData toModel(MarketDataDto dto) {
    ...
    if (dto.getData() != null) {
      List<MarketData.MarketPrice> prices =
          dto.getData().stream().map(this::toModel).collect(Collectors.toList());
  ...
  public OptimalProductionWindowDto toDto(OptimalProductionWindow window) {
    ...
    if (window.getPrices() != null) {
      List<MarketPriceDto> priceDtos =
          window.getPrices().stream().map(this::toDto).collect(Collectors.toList());
