...
public class AwattarServiceImpl implements AwattarService {
     ...
     */
    @Autowired
    public AwattarServiceImpl(AwattarApiClient apiClient, AwattarMapper mapper, AwattarConfig config) {
        ...
        if (now.isAfter(LocalTime.of(13, 0))) {
            ...
            while (!success && tries < maxTries) {
                ...
                if (data != null && data.getData() != null && data.getData().size() >= config.getProductionHours()) {
                    ...
                    tries++;
                    logger.warn("Market data for tomorrow not yet available, trying again in 120 seconds... (Attempt {}/{})", tries, maxTries);
                    try {
    ...
    public MarketData fetchCurrentMarketData() {
        ZonedDateTime nowHour = ZonedDateTime.now(ZoneId.of(VIENNA_TIMEZONE)).truncatedTo(ChronoUnit.HOURS);
    ...
    public MarketData fetchTomorrowMarketData() {
        ZonedDateTime tomorrowMidnight = ZonedDateTime.now(ZoneId.of(VIENNA_TIMEZONE))
                .plusDays(1)
    ...
    void calculateOptimalProductionWindow() {
        if (cachedMarketData == null || cachedMarketData.getData() == null || 
        ...
        double avgPricePerKwh = windowPrices.stream()
